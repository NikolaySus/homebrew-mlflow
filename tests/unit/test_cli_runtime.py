...
'''
    ...
    assert second == ()
    assert '"template_version": 8' in (tmp_path / ".homebrew-mlflow.json").read_text()
    ...
    assert "homebrew-mlflow-plugins==0.1.6" in (tmp_path / "pyproject.toml").read_text()
    assert "homebrew_mlflow_plugins-0.1.6" in (tmp_path / "uv.lock").read_text()
