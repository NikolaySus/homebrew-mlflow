...
'''
    ...
    assert second == ()
    assert '"template_version": 7' in (tmp_path / ".homebrew-mlflow.json").read_text()
    ...
    assert "homebrew-mlflow-plugins==0.1.5" in (tmp_path / "pyproject.toml").read_text()
    assert "homebrew_mlflow_plugins-0.1.5" in (tmp_path / "uv.lock").read_text()
